...
const INPUT_DIR = path.join(__dirname, 'strategy_results_wr');
const OUTPUT_FILE = path.join(__dirname, 'strategy_results_wr_combined.csv');

function parseScenarioMeta(fileName) {
  const name = path.basename(fileName, '.csv');
  const parts = name.split('_').slice(2);
  const meta = {
    strategy_group: '',
    hero_filter: 'none',
    odds_condition: 'any',
...
function main() {
  if (!fs.existsSync(INPUT_DIR)) {
    console.error('strategy_results_wr directory not found');
    process.exit(1);
  }
